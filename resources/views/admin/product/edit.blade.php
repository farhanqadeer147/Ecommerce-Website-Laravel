        ...
        <div class="card-body row">
                ...
                <form action="{{ route('productEditSave', ['product' => $product->title, 'id' => $product->id]) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="price" id="price"
                            class="form-control  @error('price') is-invalid @enderror" placeholder="1000"
                            value="{{ $product->price }}">
                        @error('price')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="variant-container">
                            @foreach ($product->productVariant as $index => $variant)
                                <div class="variant-row">
                                    ...
                                    <input type="text" name="variants[{{ $index }}][size]" placeholder="Size"
                                        class="form-control" value="{{ $variant->size }}">
                                    <input type="number" name="variants[{{ $index }}][stock]" placeholder="Stock"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stock" id="stock"
                            class="form-control  @error('stock') is-invalid @enderror" placeholder="10"
                            value="{{ $product->stock }}">
                        @error('stock')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="desc" id="desc" cols="30" class="form-control autosize @error('desc') is-invalid @enderror"
                            placeholder="Homade spicy chicken nuggets with healty chicken  . . ." required>{{ $product->desc }}</textarea>
                        @error('desc')
            ...
            <div class="col-md-6 col-12">
                <form method="post" action="{{ route('productAddImagesSave') }}" enctype="multipart/form-data"
                    class="dropzone mt-4" id="dropzone" style="border-radius:12px;">
...
@section('js')
    ...
    function addVariantRow() {
        ...
        const lastVariantIndex = parseInt(lastVariantRow.querySelector('input[type="hidden"]').value);
        const newVariantIndex = lastVariantIndex + 1;
        ...
        newVariantRow.innerHTML = `
            <input type="hidden" name="variants[${newVariantIndex}][id]" value="">
            <input type="text" name="variants[${newVariantIndex}][color]" placeholder="Color" class="form-control" value="">
            <input type="text" name="variants[${newVariantIndex}][size]" placeholder="Size" class="form-control" value="">
            <input type="number" name="variants[${newVariantIndex}][stock]" placeholder="Stock" class="form-control" value="">
            <button type="button" class="btn btn-danger btn-sm remove-variant">Remove</button>
        `;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(key, value) {
                        ...
                        data = data +
                            '<button class="btn btn-danger btn-sm delete-image-product" data-id="' +
                            value.id +
                            '" style="position:absolute;z-index:9;right:0;" onClick=alertconfirm("' +
                            value.path + '")><i class="bi bi-trash"></i></button>'
